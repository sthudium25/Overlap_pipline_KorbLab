...
dir.create(path = "OL_output")
for( i in files ){
  D <- data.frame(matrix(nrow=0,ncol=1))
  d <- read.table(here("genelists", i),header=F,stringsAsFactors=F)
  for( j in files) {
    if (j > i) {
      d2 <- read.table(here("genelists", j), header = F, stringsAsFactors = F)
      overlap <- d %>% intersect(d2)
      OL_df <- paste("D", i, j, sep = "_")
      OL_df<- assign(OL_df, rbind(D,overlap))
      myfile <- file.path("OL_output", 
...
index <- 1
for (list in list1) {
  pdf(file = paste0(here("vennOutput/"), list[[1]], "_", list[[2]], ".pdf"))
